...
class Analyzer(object):
	...
	def __init__(self, data, extruder_acceleration=None, z_acceleration=None):
		super(Analyzer, self).__init__()
		self.__data = data.decode("utf8")
	...
	def get_filament_usage(self):
		return self.__pos["E"]/1000

	def get_formatted_time(self):
		return datetime.timedelta(seconds=self.__time)

	def __process(self):
		for line in self.__data.split(os.linesep):
			if not line.startswith(";") and not line.startswith("M117"):
...
def estimate(model_file, definition_files, verbose=False):
	...
	return {"time": analyzer.get_formatted_time(), "filament": analyzer.get_filament_usage()}
